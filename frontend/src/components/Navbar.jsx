      ...
      {mobileMenuOpen && authenticated && (
        <>
          ...
          <div className="fixed top-14 right-2 w-56 bg-white rounded-lg shadow-2xl border border-gray-200 py-2 z-50 md:hidden max-h-96 overflow-y-auto">
            ...
            <Link to="/seller/products" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" onClick={() => setMobileMenuOpen(false)}>
              Meus Produtos
            </Link>
            ...
            {(user?.role === 'PRODUCER' || user?.role === 'ADMIN') && (
              <Link to="/seller/settings" className="block px-4 py-2 hover:bg-gray-100" onClick={() => setMobileMenuOpen(false)}>
                <div className="flex items-center gap-2">
                  <span>⚙️</span>
                  <div>
                    <div className="text-sm text-gray-700 font-medium">Configurações</div>
                    <div className="text-xs text-gray-500">Dados de pagamento e perfil</div>
                  </div>
                </div>
              </Link>
            )}
                  ...
                  <div className="relative" ref={userMenuRef}>
                    ...
                    {userMenuOpen && (
                      <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-10">
                        ...
                        {(user?.role === 'PRODUCER' || user?.role === 'ADMIN') && (
                          <Link to="/seller/settings" className="block px-4 py-2 hover:bg-gray-100" onClick={() => setUserMenuOpen(false)}>
                            <div className="flex items-center gap-2">
                              <span>⚙️</span>
                              <div>
                                <div className="text-sm text-gray-700 font-medium">Configurações</div>
